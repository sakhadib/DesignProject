    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Fetch problems and user details in a single request
                const { data } = await axios.get(`/user/problem/all/${id}`)
                const sortedProblems = [...data.problems].sort((a, b) => b.id - a.id)
                const recentProblems = sortedProblems.slice(0, 10) // Show latest 10 problems
